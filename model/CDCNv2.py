...
class Conv2d_X(nn.Module):
    ...
    #TODO: support non_square kernel
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3,kernel_pos = [[0,1,0],[1,1,1],[0,1,0]], stride=1,
        ...
        super().__init__()
        try:
            self.kernel_pos = torch.flatten(torch.Tensor(kernel_pos)).round()
        except:
            exit()
...
class CDCNv2(nn.Module):
    ...
    def __init__(self, basic_conv=Conv2d_X, theta=0.7, se=False,sa=False,device='cuda', **kwarg):   
        ...
        self.conv1 = nn.Sequential(
            basic_conv(3, 64, kernel_size=3, kernel_pos= [1,1,1,1,1,1,1,1,1], stride= stride, padding= padding, bias= bias, theta= theta, dilation= dilation, groups= groups, device= self.device),
            nn.BatchNorm2d(64),
            nn.ReLU(),    
        ).to(device)
